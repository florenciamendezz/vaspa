...
//                                    ]
						...
						// Capturar KPIs del Dashboard
						var kpisTotal = $('.kpi-total .kpi-value').text() || '0';
						var kpisCargados = $('.kpi-cargados .kpi-value').text() || '0';
						var kpisSinPdf = $('.kpi-sin-pdf .kpi-value').text() || '0';
						var kpisAprobados = $('.kpi-aprobados .kpi-value').text() || '0';
						var kpisRevision = $('.kpi-revision .kpi-value').text() || '0';
						var kpisDevueltos = $('.kpi-devueltos .kpi-value').text() || '0';
						var kpisRetrasados = $('.kpi-retrasados .kpi-value').text() || '0';
						var avance = $('.progress-stat-val').first().text() || '0%';
						
						// Metadatos
						var metadata = {
							columns: [
								{ text: 'Usuario Emisor: ' + emisor, fontSize: 8, italics: true },
								{ text: 'Fecha de Emisión: ' + jsDate, fontSize: 8, italics: true, alignment: 'right' }
							],
							margin: [0, 0, 0, 10]
						};
						
						var description = {
							text: 'Resumen Ejecutivo del Estado de Programas:',
							fontSize: 10,
							bold: true,
							margin: [0, 5, 0, 5]
						};
						...
						var tableTitle = {
							text: 'Detalle de Asignaturas:',
							fontSize: 10,
							bold: true,
							margin: [0, 5, 0, 5]
						};
						
						// Buscar el indice de la tabla principal
						var tableIndex = -1;
						...
						// Insertar el resumen antes de la tabla principal
						if (tableIndex !== -1) {
							doc.content.splice(tableIndex, 0, metadata, description, summaryTable, tableTitle);
						}
